import RecyclingCenter from "../classes/RecyclingCenter";
...
const findUserAdmin = async (displayLimit: number, page: number): Promise<Parse.Object[]> => {
  try {
    const query: Parse.Query = new Parse.Query(Parse.User);
...
const findUserByAdmin = async ({findBy,displayLimit, page}:{findBy: string[],displayLimit: number, page: number}): Promise<Parse.Object[]> => {
  try {
    const query: Parse.Query = new Parse.Query(Parse.User);
    query.limit(displayLimit);
    query.skip(page * displayLimit);
...
const createUserAdmin = async (params:any, currentUser: Parse.User) => {
  try {
    const User = Parse.Object.extend("_User");
    const user : Parse.User = new User();
    const {username,email,password,emailVerified,factory,firstName, middleName,lastName,nickname,aboutMe} = params;
    const values =   { "username": username,"email":email,"password":password,"emailVerified": emailVerified };
    ...
    const valuesAccount =   { "user": user,"firstName": firstName,"middleName":middleName,"lastName":lastName,"nickname": nickname,"aboutMe": aboutMe };
...
const udpdateUserAdmin = async (params:any) => {
  ...
  try{
    ...
    if(factory !== undefined){
      ...
      factory !== undefined && factory.map( async (idFactory: string) => {
        ...
        (recyclingCenterToAddTo !== undefined) && recyclingCentersRelation.add(recyclingCenterToAddTo);
